...
//	public void print(Wine wine2, CustomerAccount customer) {
	public void print(CustomerAccount customer) {
		...
		for (int i = 0; i < wine.length; i++) {
			...
			output = "Wine name: " + wine[i].getWineName() + ", Quantity: " + wine[i].getQuantity() + " units, Total Cost: £" + String.format("%.2f",wine[i].getTotalCostPerOrder()) + ", New balance: £" + String.format("%.2f", customer.getBalance())  + "\n";
			System.out.print(output);
			fw.write(output);
			}
		}	 catch(IOException e) {
